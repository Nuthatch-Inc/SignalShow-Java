...
public class CombineOpsRule {
	...
	//binary ops: tier 3 - any operations that have a higher priority than multiplication and division
...
//	public static final int[] stack_priority = { -1, 3, 1, 2, 0, 3 };
	...
	public Function combine( ArrayList<FunctionProducer> terms  ) {
		...
		if( postfix_rule_length == 0 ) return null; 
...
//		stack is used in conversion process
		...
		for( int idx = 0; idx < postfix_rule_length; idx++ ) {
			...
			case BINARY_OP_TIER_1: case BINARY_OP_TIER_2: case BINARY_OP_TIER_3: 
			...
			case UNARY_OP_TIER_1: case UNARY_OP_TIER_2: case UNARY_OP_TIER_3: 
		...
		if( stack.empty() ) return null;  //empty stack
		Function result = stack.pop(); 
		if( !stack.empty() ) return null;  //too many symbols
		return result;
