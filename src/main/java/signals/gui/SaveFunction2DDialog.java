...
public class SaveFunction2DDialog extends SaveFunctionDialog {
	...
	public boolean commitSettings() {
		...
		if (format.equals(ImageWriter.typeToExtension(ImageType.TEXT))) {
		...
		} else if (format.equals(ImageWriter.typeToExtension(ImageType.CSV))) {

			for (Part part : selectedParts) {

				returnflag &= ImageWriter.writeCSV(getSelectedFile(part.toString(), format), function, part);
			}
			...
			for (Part part : selectedParts) {
				if (part == Part.REAL_PART)
					hasReal = true;
				if (part == Part.IMAGINARY_PART)
					hasImaginary = true;
			}
