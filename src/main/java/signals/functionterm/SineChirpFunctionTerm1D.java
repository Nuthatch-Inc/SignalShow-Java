 ...
 */
public class SineChirpFunctionTerm1D extends AnalyticFunctionTerm1D {

	public SineChirpFunctionTerm1D() {
		super();
	 ...
	 * @param paramBlock
	 */
	public SineChirpFunctionTerm1D(ParameterBlock paramBlock) {
	...
	public double[] create(double[] indices) {
		...
		for (int i = 0; i < dimension; i++) {
			...
			chirp[i] = amplitude
					* Math.sin(piOverBPow * Math.pow((indices[i] - center), Order) + Math.toRadians(initialPhase));
	...
	public void initTypeModel(DataGeneratorTypeModel model) {
		...
		// default paramNames
		String[] param_names = { "A" + '\u2080', "x" + '\u2080', "\u03B1", '\u03D5' + "" + '\u2080' + " (" + '\u00B0' + ")",
				"n" };
		model.setParamNames(param_names);

		// Fixed BUG-003: Changed default alpha from 32 to 16 (sqrt(256), where 256 is
		// default dimension)
		// This makes the chirp appear properly aliased by default
		Object[] param_defaults = { new Double(1), new Double(0), new Double(16), new Double(0), new Integer(2) };
		...
		SpinnerNumberModel widthSpinnerModel = new SpinnerNumberModel(16.0, -100000.0, 100000.0, 1.0);
	...
	public String getEquation(String[] variables) {
		...
		return amplitudeMultiplierString() + "cos[\u03C0(" + formattedParamString(variables[0]) + ")^" + getOrder()
				+ StringConversions.PhaseString(getInitialPhase()) + "]";
	}
}
