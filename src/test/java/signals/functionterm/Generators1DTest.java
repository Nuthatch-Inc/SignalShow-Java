...
public class Generators1DTest {
    ...
    class RectangleFunctionTests {
        ...
        void rectWidth() {
            ...
            int count = 0;
            for (double v : result) {
                if (v > 0.5) count++;  // Exclude edge values which are 0.5
            }
    ...
    class ChirpFunctionTests {
        ...
        void chirpFrequencyIncreases() {
            // ChirpFunctionTerm1D has 5 parameters: amplitude, center, alpha (width), initial phase, order
            ParameterBlock pb = new ParameterBlock();
            ...
            for (int i = 1; i < SIZE / 2; i++) {
                if (result[i - 1] * result[i] < 0) crossingsFirst++;
            }
            for (int i = SIZE / 2 + 1; i < SIZE; i++) {
                if (result[i - 1] * result[i] < 0) crossingsSecond++;
            }
    ...
    class CombFunctionTests {
        ...
        void combHasPeriodicImpulses() {
            ...
            int impulseCount = 0;
            for (double v : result) {
                if (Math.abs(v) > 0.5) impulseCount++;
            }
